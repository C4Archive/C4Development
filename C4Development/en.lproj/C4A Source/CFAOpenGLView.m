...
{
    CVReturn result = [(CFAOpenGLView*)displayLinkContext getFrameForTime:outputTime];
    return result;
}
...
-(void)windowTitle:(id)title {
    [self.window setTitle:[CFAString nsStringFromObject:title]];  
}
...
-(CFAImage *)screenShot {
    return [CFAImage imageWithCGImage:[self screenImageFromRect:[self visibleCanvasRect]]];
}

-(CFAImage *)screenShotFromRect:(NSRect)aRect {
    return [CFAImage imageWithCGImage:[self screenImageFromRect:aRect]];
}

-(CFAImage *)canvasShot {
    return [CFAImage imageWithCGImage:[self screenImageFromRect:NSRectFromCGRect(CGRectInfinite)]];
