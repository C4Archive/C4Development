...
-(id)initWithString:(id)aString {
    ...
    [CFAGlobalStringAttributes sharedManager].drawStringsToPDF = NO;
	return self;
}
...
-(void)drawAtPoint:(NSPoint)point withAttributes:(NSDictionary *)attribs {
	...
	if([CFAGlobalStringAttributes sharedManager].drawStringsToPDF) {
        ...
        CGContextRef pdfContext = [CFAGlobalStringAttributes sharedManager].pdfContext;
...
+(void)underlineStyle:(NSInteger)style {
	...
	if(style == SINGLE || style == DOUBLE || style == THICK ) {
		[[CFAGlobalTypeAttributes sharedManager] setValue:[NSNumber numberWithInteger:style] forKey:NSUnderlineStyleAttributeName];
	} else {
		CFALog(@"underline style must be NONE, SINGLE, THICK, or DOUBLE) -> %d",style);
	}
...
+(void)strikethroughStyle:(NSInteger)style {
	if(style == NONE) {
		...
		[[CFAGlobalTypeAttributes sharedManager] setValue:[NSNumber numberWithInteger:style] forKey:NSStrikethroughStyleAttributeName];
	} else {
		CFALog(@"strikethrough style must be NONE, SINGLE, THICK, or DOUBLE) -> %d",style);
	}
}
...
+(void)beginDrawStringsToPDFContext:(CGContextRef)context {
	...
	[CFAGlobalStringAttributes sharedManager].pdfContext = context;
	[CFAGlobalStringAttributes sharedManager].drawStringsToPDF = YES;
	[CFAGlobalStringAttributes sharedManager].isClean = NO;
}

+(void)endDrawStringsToPDFContext {
	[CFAGlobalStringAttributes sharedManager].drawStringsToPDF = NO;
    [CFAGlobalStringAttributes sharedManager].pdfContext = nil;
	[CFAGlobalStringAttributes sharedManager].isClean = YES;
