...
+(CFAImage *)imageName:(NSString *)name andType:(NSString *)type {
	return [[[CFAImage alloc] initWithImageName:name andType:type] retain];
}

+(CFAImage *)imageWithCGImage:(CGImageRef)image {
    return [[[CFAImage alloc] initWithCGImage:image] retain];
}
...
-(void)kaleidoscope:(NSPoint)center count:(NSInteger)count angle:(CGFloat)angle {
	...
	count = [CFAMath constrain:(int)count min:1 max:64];
