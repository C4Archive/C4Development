...
+(id)vectorWithX:(CGFloat)x Y:(CGFloat)y Z:(CGFloat)z {
    CFAVector *v = [[[CFAVector alloc] init] vectorWithX:x Y:y Z:z];
	return v;
}
...
+(CGFloat)distanceBetweenA:(NSPoint)pointA andB:(NSPoint)pointB {
    ...
    dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
    dispatch_sync(queue, ^(void) { 
        CFAVector *a = [CFAVector vectorWithX:pointA.x Y:pointA.y Z:0];
        CFAVector *b = [CFAVector vectorWithX:pointB.x Y:pointB.y Z:0];
        value = [a distance:b];
    });
    return value;
}
...
-(CGFloat)heading {
    ...
    dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
    dispatch_sync(queue, ^(void) {
        CGFloat angle = [CFAMath atan2Y:vec3[1] X:vec3[0]]; //always against 0
        value = angle;
    });
    return value;
}
...
-(CGFloat)displacedHeading {
    if([self distance:[CFAVector vectorWithX:pVec3[0] Y:pVec3[1] Z:pVec3[2]]] > .5) {
    __block CGFloat value;
    dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
    dispatch_sync(queue, ^(void) {
        CGFloat angle = [CFAMath atan2Y:vec3[1]-pVec3[1] X:vec3[0]-pVec3[0]]; //always against 0
        value = angle;
    });
...
-(CGFloat)headingBasedOn:(NSPoint)p {
    ...
    dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
    dispatch_sync(queue, ^(void) {
        CGFloat angle = [CFAMath atan2Y:(vec3[1]-p.y) X:(vec3[0]-p.x)]; //always against 0
        value = angle;
    });
    return value;
}
