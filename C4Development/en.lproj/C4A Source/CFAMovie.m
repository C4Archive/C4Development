...
@implementation CFAMovie

@synthesize movie;
+(CFAMovie *)movieName:(id)aName {
    NSString *stringName = [CFAString nsStringFromObject:aName];
...
+(CFAMovie *)movieName:(id)aName andType:(id)aType {
    return [[[CFAMovie alloc] initWithMovieName:aName andType:aType] retain];
}
...
-(id)initWithMovieName:(id)aName andType:(id)aType {
    ...
    if(self) {
        ...
        [[[CFACanvas sharedManager].window contentView] setWantsLayer:YES];
        [[[[CFACanvas sharedManager].window contentView] layer] addSublayer:self];
        return self;
    }
    return nil;
}
...
-(void)setVolume:(CGFloat)newVolume {
    newVolume = [CFAMath constrainf:newVolume min:0 max:1];
